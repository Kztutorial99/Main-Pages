...
// ===== INFO MODAL (Topik & Legal) =====
(function() {
  const overlay  = document.getElementById('infoModalOverlay');
  const closeBtn = document.getElementById('infoModalClose');

  function openModal(tab, subtab) {
    switchMainTab(tab);
  ...
  function switchSubtab(subtab) {
    document.querySelectorAll('.legal-subtab-btn').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.subtab === subtab);
    });
    document.querySelectorAll('.legal-content').forEach(c => {
      c.classList.toggle('active', c.id === 'subtab-' + subtab);
    });
  }
  ...
  // Legal sub-tab buttons
  document.querySelectorAll('.legal-subtab-btn').forEach(btn => {
    btn.addEventListener('click', () => switchSubtab(btn.dataset.subtab));
  });

  // Footer column title triggers
  document.querySelectorAll('.footer-tab-trigger').forEach(el => {
  ...
  document.querySelectorAll('.open-modal-link').forEach(link => {
    link.addEventListener('click', e => {
      e.preventDefault();
      openModal(link.dataset.open, link.dataset.subtab || null);
    });
  });
